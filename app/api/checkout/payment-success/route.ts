...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if email was already sent (by webhook or previous call)
    if (metadata.emailSent === 'true') {
      return NextResponse.json({
        success: true,
        message: 'Download links already sent to your email',
      });
    }
    ...
    const itemsWithUrls = await Promise.all(
      itemsArray.map(async (item: { title: string; price: number; key: string }) => ({
        ...item,
