...
export async function POST(request: NextRequest) {
  try {
    ...
    // Retrieve payment intent to get metadata and status
    ...
    const itemsWithUrls = await Promise.all(
      itemsArray.map(async (item: { id: string; title: string; price: number; key: string }) => ({
        ...item,
    ...
    const msg = {
      ...
      html: generatePurchaseEmailHTML(itemsWithUrls, total, paymentIntent.id),
    };

    await sgMail.send(msg);

    console.log(`✅ Purchase email sent to ${email} for payment ${paymentIntent.id}`);
    ...
    return NextResponse.json(
      { error: 'Failed to send download links' },
      { status: 500 }
    );
  }
}
...
function generatePurchaseEmailHTML(
  items: Array<{ title: string; price: number; downloadUrl: string }>,
  total: number,
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            }" style="color: #2563eb;">${process.env.SENDGRID_FROM_EMAIL}</a></p>
